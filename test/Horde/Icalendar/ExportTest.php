...
class ExportTest extends Horde_Test_Case
{
    public function testLineFolding()
    {
        ...
        $event->setAttribute('DTSTART', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        $event->setAttribute('DTSTAMP', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        ...
        $event->setAttribute('BINARY', base64_encode('Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aliquam sollicitudin faucibus mauris amet.'), ['ENCODING' => 'b']);
        ...
        $event->setAttribute('DTSTART', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        $event->setAttribute('DTSTAMP', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        $event->setAttribute('DESCRIPTION', 'Lörem ipsüm dölör sit ämet, cönsectetüer ädipiscing elit. Aliqüäm söllicitüdin fäücibüs mäüris ämet.', ['CHARSET' => 'UTF-8']);
        $ical->addComponent($event);
        $this->assertStringEqualsFile(
        ...
        $event->setAttribute('DTSTART', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        $event->setAttribute('DTSTAMP', ['year' => 2008, 'month' => 1, 'mday' => 1], ['VALUE' => 'DATE']);
        $event->setAttribute('DESCRIPTION', 'Löremipsümdölörsitämet,cönsectetüerädipiscingelit.Aliqüämsöllicitüdinfäücibüsmäürisämet. Löremipsümdölörsitämet,cönsectetüerädipiscingelit.Aliqüämsöllicitüdinfäücibüsmäürisämet.', ['CHARSET' => 'UTF-8']);
        $ical->addComponent($event);
        $this->assertStringEqualsFile(
    ...
    public function testEscapes()
    {
        ...
        $event1->setAttribute('DTSTART', ['year' => 2005, 'month' => 5, 'mday' => 3], ['VALUE' => 'DATE']);
        $event1->setAttribute('DTSTAMP', ['year' => 2004, 'month' => 11, 'mday' => 20], ['VALUE' => 'DATE']);
        ...
        $event1->setAttribute('CATEGORIES', null, [], true, ['Foo']);

        $event2->setAttribute('UID', '20041120-8549-innerjoin-org');
        $event2->setAttribute('DTSTART', ['year' => 2005, 'month' => 5, 'mday' => 4], ['VALUE' => 'DATE']);
        $event2->setAttribute('DTSTAMP', ['year' => 2004, 'month' => 11, 'mday' => 20], ['VALUE' => 'DATE']);
        ...
        $event2->setAttribute('CATEGORIES', null, [], true, ['Foo', 'Foo,Bar', 'Bar']);
        ...
        foreach (['2.1', '3.0'] as $version) {
            ...
            $this->assertEquals(
                ['Contact', 'Test'],
                $vcard->getAttributeValues('N')
            );
            $this->assertEquals(
                ['Test,Contact'],
                $vcard->getAttributeValues('FN')
            );
            $this->assertEquals(
                ['My,Company', 'IT Department'],
    ...
    public function testQuotedParameters()
    {
        ...
        $event1->setAttribute('SUMMARY', 'Escaped Comma in Description Field');
        $event1->setAttribute('DESCRIPTION', 'There is a comma (escaped with a baskslash) in this sentence and some important words after it, see anything here?');
        $event1->setAttribute('ORGANIZER', 'mailto:mueller@example.org', ['CN' => "Klä,rc\"hen;\n Mül:ler"]);
        $ical->addComponent($event1);
        $this->assertStringEqualsFile(
    ...
    public function testTimezone()
    {
        ...
        $event->setAttribute('DTSTART', $date, ['TZID' => 'Europe/Berlin']);
        $ical->addComponent($event);
        $this->assertStringEqualsFile(
        ...
        $start = clone $date;
        $start->mday++;
        $event->setAttribute('DTSTART', $start, ['TZID' => 'Europe/Berlin']);
        $ical->addComponent($event);
        $this->assertStringEqualsFile(
    ...
    public function testSingleCategoriesV1()
    {
        ...
        $vEvent->setAttribute('CATEGORIES', null, [], true, ['Foo']);
    ...
    public function testMultipleCategoriesV1()
    {
        ...
        $vEvent = Horde_Icalendar::newComponent('VEVENT', $ical);
        $vEvent->setAttribute('UID', '5b3be1a5-d1b0-43c1-bad4-6c29eebab01e');
        $vEvent->setAttribute('CATEGORIES', null, [], true, ['Foo', 'Bar']);
        $vEvent->setAttribute('DTSTART', $date);
        $ical->addComponent($vEvent);
    ...
    public function testDuration0()
    {
        ...
        $vevent->setAttribute('DTSTART', ['year' => 2015, 'month' => 7, 'mday' => 1], ['VALUE' => 'DATE']);
        $vevent->setAttribute('DTSTAMP', ['year' => 2015, 'month' => 7, 'mday' => 1], ['VALUE' => 'DATE']);
        $vevent->setAttribute('DURATION', 0);
        $ical->addComponent($vevent);
        $valarm = Horde_Icalendar::newComponent('VALARM', $vevent);
        $valarm->setAttribute('TRIGGER', 0, [
            'VALUE' => 'DURATION',
            'RELATED' => 'START',
        ]);
    ...
    public function testDuration8d_not_1w1d()
    {
        ...
        $ical->addComponent($vevent);
        $valarm = Horde_Icalendar::newComponent('VALARM', $vevent);
        $valarm->setAttribute('TRIGGER', -7 * 86400, [
            'VALUE' => 'DURATION',
            'RELATED' => 'START',
        ]);
